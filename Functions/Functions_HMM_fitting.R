...
viterbi_trajectory_to_rds  <- function(data_hmm, output_file, individual_info_file) {
  ...
  # ---- PARK (ajout minimal) ----
  # on relit le .rds "Night_park" qui contient park par jour
  alpage_now <- data_save$alpage[1]
  nightpark_rds <- file.path(output_dir, "3. Night_park",
                             paste0("Catlog_", YEAR, "_night_park_", alpage_now, ".rds"))
  park_data <- readRDS(nightpark_rds)
  
  
  park_day <- park_data %>%
    dplyr::mutate(day = as.Date(time)) %>%
    dplyr::distinct(ID, day, park)
  
  data_save <- data_save %>%
    dplyr::mutate(day = as.Date(time)) %>%
    dplyr::left_join(park_day, by = c("ID", "day")) %>%
    dplyr::select(-day)
  # -----------------------------
...
par_HMM_fit <- function(data, run_parameters, ncores, individual_info_file, sampling_period, output_dir) {
  ...
  # Loading libraries and functions into the clusters
  ...
  results <- parLapply(clus, unique(data$ID), function(ID) {
    # Retrieval of the alpine pasture for this individual
    alpage <- get_individual_alpage(ID, individual_info_file)
    print(paste0("ID: ", ID, " - Alpage récupéré : ", alpage))
    
    # Retrieve the sampling period specific to the individual
    sampling_period_ind <- get_individual_info(ID, individual_info_file, "Periode_echantillonnage")
    
    # Directly use the provided folder (output_dir) to store the PDF
    pdf_folder <- output_dir
    
    # Call the hmm_fit function while passing pdf_folder
...
hmm_fit <- function(data, runPar, output_dir, sampling_period) {
  ...
  # Extraction of the individual's ID
  ID = data$ID[1]
  ...
  if(runPar$knownRestingStates) {
    # If certain periods are considered as rest, define the known states
  ...
  # Calculation of the most probable states and their probabilities
...
par_HMM_fit <- function(
    data,
    run_parameters,
    ncores,
    individual_info_file,
    output_dir,
    target_sampling_period = 600
) {
  
  # target_sampling_period = période finale souhaitée, en secondes
  # 600 secondes = 10 minutes
  
  print("+++ momentuHMM parallel RUN +++")
  startTime <- Sys.time()
  
  clus <- snow::makeCluster(ncores, outfile = "")
  
  # Arrêter proprement le cluster même en cas d'erreur
  on.exit(
    try(snow::stopCluster(clus), silent = TRUE),
    add = TRUE
  )
  
  snow::clusterExport(
    clus,
    as.list(lsf.str(.GlobalEnv))
  )
  ...
  # Chargement des packages et fonctions sur chaque cœur
  snow::clusterCall(
    clus,
    function() {
  ...
  results <- snow::parLapply(
    clus,
    unique(data$ID),
    function(ID) {
      
      # Alpage de l'individu
      alpage <- get_individual_alpage(
        ID,
        individual_info_file
      )
      
      # Période d'échantillonnage propre au collier :
      # 120 secondes pour les Catlog
      # 300 secondes pour les Followit
      sampling_period_ind <- as.numeric(
        get_individual_info(
          ID,
          individual_info_file,
      ...
      # Vérification de la métadonnée
      if (
        length(sampling_period_ind) != 1 ||
        is.na(sampling_period_ind) ||
        !is.finite(sampling_period_ind) ||
        sampling_period_ind <= 0
      ) {
        stop(
          "Période d'échantillonnage incorrecte pour le collier ",
          ID,
          " : ",
          sampling_period_ind
        )
      ...
      # Calcul du facteur nécessaire pour atteindre 10 minutes
      resampling_ratio_ind <-
        target_sampling_period / sampling_period_ind
      
      # Le facteur doit être entier :
      # 600 / 120 = 5
      # 600 / 300 = 2
      if (
        resampling_ratio_ind < 1 ||
        abs(
          resampling_ratio_ind -
          round(resampling_ratio_ind)
        ) > 1e-8
      ) {
        stop(
          "Le collier ", ID,
          " a une période de ", sampling_period_ind,
          " secondes, incompatible avec une période finale de ",
          target_sampling_period,
          " secondes."
        )
      }
      ...
      resampling_ratio_ind <-
        as.integer(round(resampling_ratio_ind))
      
      # Copie individuelle des paramètres
      run_parameters_ind <- run_parameters
      
      # Modification uniquement du facteur de rééchantillonnage
      run_parameters_ind$resampling_ratio <-
        resampling_ratio_ind
      ...
      hmm_fit(
        ...
        runPar = run_parameters_ind,
        output_dir = output_dir,
        sampling_period = sampling_period_ind
      )
    }
  )
